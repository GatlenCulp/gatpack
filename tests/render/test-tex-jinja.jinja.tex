\documentclass[12pt]{article}

% Define Jinja placeholder commands for better editor visualization
\usepackage{xcolor}
\definecolor{jinjaColor}{HTML}{7B68EE}  % Medium slate blue color for Jinja
\definecolor{jinjaVarBg}{HTML}{E6E6FA}    % Light lavender for variables
\definecolor{jinjaBlockBg}{HTML}{FFE4E1}  % Misty rose for blocks
\definecolor{jinjaCommentBg}{HTML}{E0FFFF}  % Light cyan for comments
\newcommand{\VAR}[1]{\colorbox{jinjaVarBg}{#1}}
\newcommand{\BLOCK}[1]{\colorbox{jinjaBlockBg}{#1}}
\newcommand{\COMMENT}[1]{\colorbox{jinjaCommentBg}{#1}}

\usepackage[margin=1in]{geometry}
\usepackage[T1]{fontenc}
\usepackage{graphicx}  % Required for including images
\usepackage[scaled]{helvet} % Helvetica font package
\renewcommand{\familydefault}{\sfdefault} % Make Helvetica the default font
\usepackage{array}  % For table formatting
\usepackage{hyperref}  % Add this with other packages
\hypersetup{
    colorlinks=false,  % Disable colored links
    pdfborder={0 0 0} % Remove link borders
}

\usepackage{sectsty}        % Allows customizing section fonts
\allsectionsfont{\sffamily} % Make section titles sans-serif


\begin{document}
% ━━━━━━━━━━━━━━━━━━━━━━━━ Define Variables ━━━━━━━━━━━━━━━━━━━━━━━━ %
\def\logopath{maia-horizontal_cropped.jpeg}  % Changed to relative path

% These variables will be replaced by Jinja
\newcommand{\programlongname}{\VAR{program_long_name}}
\newcommand{\timeperiod}{\VAR{time_period}}
\newcommand{\chroninfo}{\VAR{chron_info}}
\newcommand{\maintitle}{\VAR{title}}  % Changed from \title to avoid conflict
\newcommand{\mainsubtitle}{\VAR{subtitle}}  % Changed from \subtitle for clarity

% Define custom colors from JSON
\definecolor{primaryColor}{HTML}{B48BFF}
\definecolor{primaryFaded}{HTML}{AEA3CA}

% Define core readings array with URLs
\def\firstReading{\href{https://www.anthropic.com/index/anthropics-responsible-scaling-policy}{Anthropic's responsible scaling policy \newline \emph{(Anthropic, 2023)}}}
\def\secondReading{\href{https://www.lesswrong.com/posts/vgHPAXWBkjBgaHe4P/scaling-ai-safely-can-preparedness-frameworks-pull-their}{Scaling AI Safely: Can Preparedness Frameworks Pull Their Weight? \newline \emph{(Jack Titus, 2024)}}}
\def\thirdReading{\href{https://www.anthropic.com/index/the-long-term-benefit-trust}{The Long-Term Benefit Trust \newline \emph{(Anthropic, 2023)}}}
\def\fourthReading{\href{https://hbr.org/2023/12/ai-is-testing-the-limits-of-corporate-governance}{AI is Testing the Limits of Corporate Governance \newline \emph{(Tallarita, 2023)}}}

% ━━━━━━━━━━━━━━━━━━━━━━ Document Begins Here ━━━━━━━━━━━━━━━━━━━━━━ %

\thispagestyle{empty} % Remove page numbering

% ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ Header ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
\begin{center}
    \includegraphics[width=0.5\textwidth]{\logopath}\\
    \vspace{0.5em}
    {\Large \textcolor[HTML]{333333}{\programlongname}}\\
    \vspace{0.5em}
    {\textcolor[HTML]{333333}{\timeperiod}}
\end{center}

\vspace{8em}

% ━━━━━━━━━━━━━━━━━━━━━━━━━━━ Main Title ━━━━━━━━━━━━━━━━━━━━━━━━━━━
\begin{center}
    [ \chroninfo ]\\
    \vspace{0.7em}
    {\Huge \textbf{\maintitle}}\\
    \vspace{0.7em}
    \uppercase{\mainsubtitle}
\end{center}

\vfill  % Push the following content to the bottom

% ━━━━━━━━━━━━━━━━━━━━━━━━━━━━ Readings ━━━━━━━━━━━━━━━━━━━━━━━━━━━━
\begin{center}
    \begin{tabular}{>{\centering\arraybackslash\footnotesize\color{primaryFaded}}p{0.45\textwidth}>{\centering\arraybackslash\footnotesize\color{primaryFaded}}p{0.45\textwidth}}
        \firstReading & \secondReading \\
        \thirdReading & \fourthReading
    \end{tabular}
\end{center}

\vspace{2em}  % Add some space at the bottom margin

\end{document}